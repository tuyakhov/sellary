...
contract SellaryRenderer is Initializable {
  ...
  function metadata(
    ...
    RenderVars memory renderVars = RenderVars({
        decFlowRate: StringDecimals.cutAtPrecision(StringDecimals.decimalString(uint256(uint96(nftFlowrate)), 18, false), 6),
        tokenSymbol: tokenSymbol_,
        decYield: StringDecimals.cutAtPrecision(StringDecimals.decimalString(dueValue, 18, false),4),
            ...
            abi.encodePacked("data:application/json;base64,", 
                Base64Upgradeable.encode(
                    abi.encodePacked(
                        ...
                        bytes('","attributes": [{"display_type": "date", "trait_type": "expires","value":'),
...
function renderBG() internal pure returns (bytes memory bg) {
  ...
  function renderSVG( 
        ...
        return abi.encodePacked('<svg width="600" height="600" xmlns="http://www.w3.org/2000/svg">',
            ...
            '<text text-anchor="start" font-size="50" x="28" y="120" font-family="Arial" fill="white">', bytes(renderVars.decFlowRate), bytes(' '),bytes(renderVars.tokenSymbol),'/s</text>',
            '<text text-anchor="start" font-size="30" x="28" y="200" font-family="Arial" fill="black"> Yield </text>', 
            '<text text-anchor="start" font-size="50" x="28" y="260" font-family="Arial" fill="white">', renderVars.decYield,bytes(' '),renderVars.tokenSymbol,'</text>', 
            '<text text-anchor="start" font-size="30" x="28" y="340" font-family="Arial" fill="black"> Expiry Date (Y/M/D)</text>', 
            '<text text-anchor="start" font-size="50" x="28" y="400" font-family="Arial" fill="white">',bytes(renderVars.untilDateString),'</text>',
            '</svg>'
        );
    }
}